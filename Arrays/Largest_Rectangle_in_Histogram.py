'''84. Largest Rectangle in Histogram
Hard
Topics
premium lock icon
Companies
Given an array of integers heights representing the histogram's bar height where the width of each bar is 1, return the area of the largest rectangle in the histogram.

 

Example 1:


Input: heights = [2,1,5,6,2,3]
Output: 10
Explanation: The above is a histogram where width of each bar is 1.
The largest rectangle is shown in the red area, which has an area = 10 units.
Example 2:


Input: heights = [2,4]
Output: 4
 

Constraints:

1 <= heights.length <= 105
0 <= heights[i] <= 104'''

class Solution(object):
    def largestRectangleArea(self, heights):
        """
        :type heights: List[int]
        :rtype: int"""
        # Store the largest rectangle found so far
        max_area = 0

        # Try every bar as the minimum height
        for i in range(len(heights)):

            # Current bar's height
            current_height = heights[i]

            # Initially, the rectangle contains only this bar
            left = i
            right = i

            # -----------------------------
            # Expand towards the LEFT
            # -----------------------------
            while left > 0 and heights[left - 1] >= current_height:
                left -= 1

            # -----------------------------
            # Expand towards the RIGHT
            # -----------------------------
            while right < len(heights) - 1 and heights[right + 1] >= current_height:
                right += 1

            # Width of the rectangle
            width = right - left + 1

            # Area using the current bar as the minimum height
            area = current_height * width

            # Update maximum area
            max_area = max(max_area, area)

        return max_area
    



sol = Solution()
heights = list(map(int, input().split()))
print(sol.largestRectangleArea(heights))